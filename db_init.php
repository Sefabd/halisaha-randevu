<?php
// db_init.php - Database Schema Rebuilder with Features, Maintenance & Weekend Hours
...
try {
    ...
    // 3. Facilities Table (Added features, closed_dates, weekend hours)
    ...
    // 4. Facility Fields Table (Added status & features)
    ...
    $today = date('Y-m-d');
    $insRes = $pdo->prepare("INSERT INTO field_reservations (facility_id, field_id, field_name, team_name, contact_name, phone, reservation_date, reservation_time, fee, status, subscription_plan) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

    $insRes->execute([$fac1_id, 1, 'Saha 1', 'Moda Gençlik', 'KAPTAN ALİ', '05321112233', $today, '11:00', 1200.00, 'Onaylandı', 'Standart']);
    $insRes->execute([$fac1_id, 1, 'Saha 1', 'Fenerbahçe Veteran', 'SERKAN HOCA', '05352223344', $today, '12:00', 1200.00, 'Onaylandı', 'Aylık Fix']);
    $insRes->execute([$fac1_id, 2, 'Saha 2', 'Kadıköy Gücü', 'CANER ERTEKİN', '05334445566', $today, '14:00', 1100.00, 'Onaylandı', 'Standart']);
