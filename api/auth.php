<?php
// api/auth.php - Session & Authentication Controller
...
try {
    if ($action === 'login_owner') {
        ...
        if ($facility && password_verify($password, $facility['password'])) {
            ...
            // Check fallback plain text '123' if hash comparison fails on fresh DB
            if ($facility && $password === '123') {
                $_SESSION['user_role'] = 'owner';
                $_SESSION['facility_id'] = $facility['id'];
                $_SESSION['facility_name'] = $facility['name'];
                $_SESSION['owner_name'] = $facility['owner_name'];
